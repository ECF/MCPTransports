...
Upon startup, an MCP Server (myproject.mcpserver) is launched on localhost so that this client can communicate with it via stdio transport.   The code for this launching is in [McpClientComponent](src/main/java/com/composent/ai/mcp/examples/arithmetic/testclient/McpClientComponent.java).
